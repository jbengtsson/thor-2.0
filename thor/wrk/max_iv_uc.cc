...
{
  int          n_b4, i, j, m;
  double       chi2_old, fret, g2, gamma, dg2;
  double       **A, *b, *bn0;
  double       twoJ[2];
  tps          K_re, K_im;
  ss_vect<tps> nus;
  ...
  dvcopy(bn_prms.bn, n_b4, bn0);

  if (n_iter == 1) {
    for (i = 1; i <= n_b4; i++) {
      g_[i] = bn_prms.dbn[i]; h_[i] = g_[i];
    }
  } else {
    dg2 = g2 = 0e0;
    for (i = 1; i <= n_b4; i++) {
  ...
  db4_max = 0e0;
  for (i = 1; i <= n_b4; i++) {
    db4_max = max(fabs((bn_prms.bn[i]-bn0[i])), db4_max);
    printf("%11.3e", bn_prms.bn[i]-bn0[i]);
  }
  printf("\n");

...
void prt_bn(const param_type &bn_prms)
{
  ...
  k = 0;
  fprintf(outf, "\nsf1:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);
  k++;
  fprintf(outf, "sf2:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);
  k++;
  fprintf(outf, "sf3:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);

  k++;
  fprintf(outf, "\nsd1:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);
  k++;
  fprintf(outf, "sd2:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);

  k++;
  fprintf(outf, "\nss3:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);
  k++;
  fprintf(outf, "ss4:  sextupole, L = 0.0,  K = %8.5f, N = 1"
	  ", Method = Meth;\n", bn_prms.bn_scl[k]*bn_prms.bn[k+1]);
...
void fit_nu(const double nu_x, const double nu_y)
{
  int          n_bn, i, m;
  double       **A, *b;
  ss_vect<tps> nus;
  ...
  for (i = 1; i <= n_bn; i++) {
    ...
    m = 0;
    A[++m][i] = get_a(1e0, nus[3], 0, 0, 0, 0, 0);
    A[++m][i] = get_a(1e0, nus[4], 0, 0, 0, 0, 0);

    bn_prms.clr_prm_dep(i-1);
  }

  m = 0;
  b[++m] = -(get_b(1e0, nus[3], 0, 0, 0, 0, 0)-nu_x);
  b[++m] = -(get_b(1e0, nus[4], 0, 0, 0, 0, 0)-nu_y);
...
int main(int argc, char *argv[])
{
  ...
  // Center of straight.
  const double beta[]  = {17.7, 0.59},
               A_max[] = {20e-3, 8e-3}, delta = 1e-2;
  ...
  if (false) {
    bn_prms.add_prm("qf01", 2, 1e1, 1.0);
    bn_prms.add_prm("qf02", 2, 1e1, 1.0);
    bn_prms.add_prm("qf03", 2, 1e1, 1.0);

    bn_prms.add_prm("qd01", 2, 1e1, 1.0);
    bn_prms.add_prm("qd02", 2, 1e1, 1.0);
    bn_prms.add_prm("qd03", 2, 1e1, 1.0);
    bn_prms.add_prm("qd04", 2, 1e1, 1.0);
  ...
  // bn_prms.add_prm("sb",  3, 1e2, 1.0);
  // bn_prms.add_prm("sb1", 3, 1e2, 1.0);
  // bn_prms.add_prm("sb2", 3, 1e2, 1.0);

  // bn_prms.add_prm("ss1", 3, 1e2, 1.0);

  // bn_prms.add_prm("eq02", 3, 1e2, 1.0);
  // bn_prms.add_prm("eq05", 3, 1e2, 1.0);

  bn_prms.add_prm("sf1", 3, 1e2, 1.0);
  bn_prms.add_prm("sf2", 3, 1e2, 1.0);
  bn_prms.add_prm("sf3", 3, 1e2, 1.0);

  bn_prms.add_prm("sd1", 3, 1e2, 1.0);
  bn_prms.add_prm("sd2", 3, 1e2, 1.0);

  bn_prms.add_prm("ss3", 3, 1e2, 1.0);
  bn_prms.add_prm("ss4", 3, 1e2, 1.0);

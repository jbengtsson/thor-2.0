...
void get_dnu2(const tps &K_re, tps &dnu2)
{
  ...
  const bool prt = false, threeD = false;
...
void H_zero(const double eps)
{
  ...
  bn = dvector(1, n_prm); xi = dmatrix(1, n_prm, 1, n_prm);
  ...
  dpowell(bn, xi, n_prm, eps, &iter, &fret, f_nl);

  cout << endl;
  cout << scientific << setprecision(1)
       << setw(2) << iter << ", chi2: " << fret << endl;

  free_dvector(bn, 1, n_prm); free_dmatrix(xi, 1, n_prm, 1, n_prm);
}

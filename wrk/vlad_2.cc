...
// Program to design the linear optics for a periodic 4-cell 2nd order achromat
// with a dispersion wave.
// Use A-script to propagate initial conditions for dispersion.
// Cell phase advanced is adjusted.
// Dispersion is controlled by intial conditions.
...
const double b2L_max = 100e0, dL_max = 0.2;

// ARC3.
const double alpha_[] = {0e0, 0e0}, beta_[]  = {4.91574, 1.55124};
const double eta_[]   = {0.25725, 0e0}, etap_[]  = {0e0, 0e0};
...
double f_opt(double b2s[])
{
  ...
  m_cstr = 0;
  b[++m_cstr] = h_ijklm(Map[x_],  1, 0, 0, 0, 0) + 1e0/sqrt(2e0);
  b[++m_cstr] = h_ijklm(Map[y_],  0, 0, 1, 0, 0) - 1e0/sqrt(2e0);
...
void get_prms(void)
{
  ...
  // Arc3.
  n_prm = 0;
  bn_prm[n_prm].Fnum =  get_Fnum("b1h"); bn_prm[n_prm++].n = Quad;
  bn_prm[n_prm].Fnum =  get_Fnum("q0h"); bn_prm[n_prm++].n = Quad;
  // bn_prm[n_prm].Fnum =  get_Fnum("dd0"); bn_prm[n_prm++].n = 0;

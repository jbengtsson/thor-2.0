...
*           2: TYPE (1:REAL, 2:integerCHARACTER, 4:DA)
*           3: ADDRESS
...
*
      ...
      integerction str_len(str)

      character  str*(*)
      integern

      n = len(str)
 10   if (str(n:n) .eq. ' ') then
         n = n - 1
         goto 10
      endif
...
*
         DO 30 I=1,INAM
            ...
            IF(ID.EQ.0) THEN
               WRITE(2,'(A)') '  DAvect  '
     +              //CNAM(I)(1:str_len(cnam(i)))//';'
            ELSE
               WRITE(2, '(2A, 44A1, 5(/''     *   '', 60A1))')
     *              '  DAvect  ', CNAM(I)(1:str_len(cnam(i))), '[',
...
*
         WRITE(2, *)
         WRITE(2,'(2X,A)') 'DAvect  isdrda, iscrri, idao;'
         WRITE(2,'(2X,A)') 'double  rscrri;'
         WRITE(2, *)
!         WRITE(2,'(6X,A)') 'common /dascr/ isdrda(100), rscrri(100)'
!     *                         //', iscrri(100), idao'
*
         DO 60 I=1,INAM
...
cfrs         READ(1,'(A6,i1,1X,A64)',END=60) PREC,INITIAL,IDAT
         ...
         IF(PREC(1:4).ne.'//DA'.and.(INITIAL.ne.0.or.INITIAL.ne.1)) then
           write(6,*) 'Sorry, you are not using the format introduced ', 
     *     'by F.Schmidt'//' which ',
     *     'requires an additional line right after the line: ',  
     *     '//DA  E D; for a properly allocation'//
     *     ' and also deallocation of variables. '//
     *     'Please insert a line: //DA 0 or ',
     *     'e.g.: //DA 1 if(ierr.eq.0.or.ierr.eq.2) then '//
...
*
     ...
     *           (NPAR(I,6).NE.1)) then
               IF(ID.EQ.0) THEN
                  WRITE(2, '(11A)')
     *                 '  DAall(', CNAM(I)(1:str_len(cnam(i))), ',',
     +                 blank(str_len(cnam(i))+1:8), ' 1, ''', CNAM(I),
     +                 ''', ',
     ...
     +                 ');'
               ELSE
                  WRITE(2, '(13A)')
     *                 '  DAall(', CNAM(I)(1:str_len(cnam(i))), ',',
     +                 blank(str_len(cnam(i))+1:8), ' ',
     *                 ((CTEX(IT+J+2)(K:K),
     +                 K=1,ILAST(CTEX(IT+J+2),1,80)), J=1,ID),
     *                 ', ''', CNAM(I), ''', ',
     +                 (CTEX(IT+1)(K:K), K=1,ILAST(CTEX(IT+1),1,80)),
     *                 ', ',
     +                 (CTEX(IT+2)(K:K), K=1,ILAST(CTEX(IT+2),1,80)),
     +                 ');'
               ...
               if(icount.gt.mname) then
                  write(2,*)
     +                 'C   Number of variables to be allocated and ' 
                  write(2,*)
     +                 'C   deallocated is larger than the parameter ',
     *                 'MNAME: ', MNAME
     ...
     *                 'MNAME: ',MNAME
               ...
               IF(ID.EQ.0) THEN
                  write(NAMEDAL(icount), '(5A)')
     *                 '  DAdal(', CNAM(I)(1:str_len(cnam(i))),
     +                 ',', blank(str_len(cnam(i))+1:8), ' 1);'
               ELSE
                  write(NAMEDAL(icount), '(7A)')
     *                 '  DAdal(', CNAM(I)(1:str_len(cnam(i))),
     +                 ',', blank(str_len(cnam(i))+1:8),' ',
     +                 ((CTEX(IT+J+2)(K:K),
     +                 K=1,ILAST(CTEX(IT+J+2),1,80)), J=1,ID), ');'
...
*
      ...
      ELSEIF(CMOD.EQ.'PRECOM') THEN
         ...
         if(alin(IAMAX+2:IAMAX+6).eq.'dadal') then
           if(icount.gt.0) then
             do 70 i=icount,1,-1
               write(2,'(a)') namedal(i)(1:str_len(namedal(i)))
 70          continue
           endif
         endif
*
...
*
     ...
     *       PREC(1:4).EQ.'//DA'.OR.PREC(1:3).EQ.'*DA') THEN
            DO 30 I=1,INAM
   ...
   25       CONTINUE
     ...
     *                   ALIN(IPOS-2:IPOS-2)).NE.0).OR.
     +            (INDEX('ABCDEFGHIJKLMNOPQRSTUVWXYZ_1234567890',
     *                   ALIN(IPOS-2:IPOS-2)).NE.0)) GOTO 25
            ENDIF
            IF(IPOS+ICN-1.NE.IAMAX+67) THEN
               IF((INDEX('abcdefghijklmnopqrstuvwxyz',
     *                   ALIN(IPOS+ICN-1:IPOS+ICN-1)).NE.0).OR.
     +            (INDEX('ABCDEFGHIJKLMNOPQRSTUVWXYZ_1234567890',
...
*
     ...
     *         REST(1:ILAST(REST,1,8))
         ...
         ENDIF
         IF(PREC(1:4).EQ.'//DA'.OR.PREC(1:3).EQ.'*DA')
     *      IAMAX = IAMAX + ILL
*
*     DIRECT COMPILE MODE
*     *******************
...
*
      ...
      IF((INDEX('abcdefghijklmnopqrstuvwxyz',A(IA1:IA1)).EQ.0).AND.
     +   (INDEX('ABCDEFGHIJKLMNOPQRSTUVWXYZ',A(IA1:IA1)).EQ.0)) RETURN
*
      ICH = 0
*
      DO 100 I=IA1+1,IA2
         IF((INDEX('abcdefghijklmnopqrstuvwxyz',A(IA1:IA1)).EQ.0).AND.
     +      (INDEX('ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_',A(IA1:IA1))
     +      .EQ.0)) THEN
            RETURN
         ELSE
            ICH = ICH + 1
...
*
      ...
      CHARACTER BLANK*8,NUM*10,LET*52,SEARCH*8,AER*50
...
*
      DATA NUM  / '1234567890' /
      DATA LET  /'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'/
      DATA AER  / '                                                  ' /
      DATA IFI  / 0 /
*
      IF(IFI.EQ.0) THEN
...
!     C: convert brackets to parenthesis
      DO 12 I = 1, IA
         IF (A(I:I).EQ.'[') THEN
            A(I:I) = '('
...
*
      PARAMETER (LSCR=100)
      CHARACTER A*800,AL*808,AR*800,AS*800,AC*800,BLANK*800,ABC*52
      character line*80
      integerRTY(LSCR),INDEX(7), str_len
*
      SAVE ICALL
      SAVE BLANK
*
...
*
         IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
            ...
            ELSEIF(ITS.EQ.4) THEN
               A = 'DAcon('//AS(1:LS)//', '//AR(1:LR)//')'
            ELSE
               GOTO 1000
            ENDIF
         ELSEIF(ITR.EQ.4) THEN
            IF(ITS.EQ.4) THEN
               A = 'DAcop('//AR(1:LR)//', '//AS(1:LS)//')'
            ELSE
               GOTO 1000
            ENDIF
         ELSE
...
*        -----------------
     ...
     *                 AS(1:LS)//')'
            ...
            ELSEIF(ITL.EQ.4) THEN
               IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
                  ITS = 4
                  A = 'DAcad('//AL(1:LL)//', '//AR(1:LR)//', '//
     *                 AS(1:LS)//')'
               ELSEIF(ITR.EQ.4) THEN
                  ITS = 4
                  A = 'DAadd('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSE
                  GOTO 1000
               ENDIF
...
*        ---------------------
     ...
     +                 //AS(1:LS)//')'
            ...
            ELSEIF(ITL.EQ.4) THEN
               IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
                  A = 'DAcsu('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSEIF(ITR.EQ.4) THEN
                  A = 'DAsub('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSE
                  GOTO 1000
               ENDIF
...
*        ---------------------
     ...
     +                 //AS(1:LS)//')'
            ...
            ELSEIF(ITL.EQ.4) THEN
               IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
                  A = 'DAcmu('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSEIF(ITR.EQ.4) THEN
                  A = 'DAmul('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSE
                  GOTO 1000
               ENDIF
...
*        ---------------------
     ...
     +                 //AS(1:LS)//')'
            ...
            ELSEIF(ITL.EQ.4) THEN
               IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
                  A = 'DAcdi('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSEIF(ITR.EQ.4) THEN
                  A = 'DAdiv('//AL(1:LL)//', '//AR(1:LR)//', '
     +                 //AS(1:LS)//')'
               ELSE
                  GOTO 1000
               ENDIF
...
*        ---------------------
     ...
     *                 //AS(1:LS)//')'
            ...
            ELSEIF(ITL.EQ.4) THEN
               IF((ITR.EQ.1).OR.(ITR.EQ.2)) THEN
                  A = 'DAexc('//AL(1:LL)//', '//AR(1:LR)//', '
     *                 //AS(1:LS)//')'
               ELSEIF(ITR.EQ.4) THEN
                  A = 'DAexx('//AL(1:LL)//', '//AR(1:LR)//', '
     *                 //AS(1:LS)//')'
               ELSE
                  GOTO 1000
               ENDIF
...
*
            ...
            ELSE
               IF(KFUN(-IOP).EQ.1) THEN
                  A = 'DAfun('//''''//FUNC(-IOP)(1:4)//''''//','
     +                 //AR(1:LR)//', '//AS(1:LS)//')'
               ELSE
                  A = 'DAfun2('//''''//FUNC(-IOP)(1:4)//''''//', '
...
*     ****************************
*
      ILIN = ILAST(A,1,800)/66 + 1
      do 40 j = 1, ilin
         line = a((j-1)*66+1:j*66)
         if ((j .ne. ilin) .and. (ilin > 1)) then
            WRITE(2,'(2A)') '  ', line(1:str_len(line))
         else
            WRITE(2,'(3A)') '  ', line(1:str_len(line)), ';'
         endif
 40   continue
      ILIN = ILAST(AC,1,800)/66 + 1
      IF(ICON.NE.0) THEN
         do 50 j = 1, ilin
            line = ac((j-1)*66+1:j*66)
            if ((j .ne. ilin) .and. (ilin > 1)) then
               WRITE(2,'(2A)') '  ', line(1:str_len(line))
            else
               WRITE(2,'(3A)') '  ', line(1:str_len(line)), ';'
...
*
      PARAMETER (LSCR=100)
      CHARACTER A*(*),CNUM(100)*2, blank*8
      integerRTY(*),IN(*), str_len, k
*
...
*
      IF(IA.LT.0) THEN
         ...
         IF(ITA.EQ.1) THEN
            k = str_len(CNUM(IA))
            LA = 13 + k
            A(1:LA) = 'rscrri['//CNUM(IA)(1:k)//'+idaa]'
         ELSEIF(ITA.EQ.2) THEN
            k = str_len(CNUM(IA))
            LA = 13 + k
            A(1:LA) = 'iscrri['//CNUM(IA)(1:k)//'+idaa]'
         ELSE
            k = str_len(CNUM(IA))
            LA = 13 + k
            A(1:LA) = 'isdrda['//CNUM(IA)(1:k)//'+idaa]'
         ENDIF
*
...
*
      ...
      IF(II.LT.0) THEN
         ...
         IF(ITI.EQ.1) THEN
            k = str_len(CNUM(IA))
            A(LA+1:LA+13+k) = 'rscrri['//CNUM(II)(1:k)//'+idaa'
            LA = LA + 13 + k
         ELSEIF(ITI.EQ.2) THEN
            A(LA+1:LA+13+k) = 'iscrri['//CNUM(II)(1:k)//'+idaa]'
            LA = LA + 13 + k
         ELSE
            A(LA+1:LA+13+k) = 'isdrda['//CNUM(II)(1:k)//'+idaa]'
            LA = LA + 13 + k
         ENDIF
*
      ELSEIF(II.LE.LNAM.AND.II.GE.1) THEN
         k = str_len(CNUM(IA))
         A(LA+1:LA+k) = CNAM(II)(1:k)
         LA = LA + k
         ITI = NPAR(II,2)
...
!         IL = IL + 2
      ...
      IF((NPAR(IA,1).EQ.1).AND.(ITI.NE.1).AND.(ITI.NE.2)) THEN
         WRITE(6,'(1X,A)') '### ERROR, ARRAY INDEX NOT integerREAL'
         WRITE(2,'(1X,A)') '### ERROR, ARRAY INDEX NOT integerREAL'
         RETURN
      ENDIF
*
